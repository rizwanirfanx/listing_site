...
class InlineRequestController extends Controller
{
	...
	public function make($table, $field, Request $request)
	{
        ...
        if(isset($request->field) && $request->field=='verified_top'){
           $fieldValue= $request->fieldValue;
            	$dat=DB::table('posts')
              ->where('id', $primaryKey)
              ->update(['verified_top' => $fieldValue]);
        }
		...
		else if ($table == 'payments' && $field == 'active') 
		{
			...
			if ($item->{$field} == 1) 
			{
			    $model::where('id',$primaryKey)->update(['updated_at' => date('Y-m-d h:i:s')]);
			...
			{
    		    $model::where('id',$primaryKey)->update(['updated_at' => NULL]);
