...
class CreateController extends FrontController
{
	...
	public function postPostStep(PostRequest $request)
	{
		...
		$nextUrl = qsUrl($nextUrl, request()->only(['package','final_price','duration']), null, false);
	...
	public function getPicturesStep(Request $request)
	{
		if ($this->step($request) < 1) {
			...
			$backUrl = qsUrl($backUrl, request()->only(['package','final_price','duration']), null, false);
	...
	{		
        ...
        $wallet=mywallet::where('user_id','=',$user_id)->sum('price');
		}else{
		    $wallet=0;
		}
		$phone=PaymentMethod::select('phone_number')->where('id','=',5)->get()->toArray()[0]['phone_number'];
		
		return appView('post.createOrEdit.multiSteps.packages.create', compact('payment','wallet','phone'));
	}
	...
	public function postPaymentStep(PackageRequest $request)
	{
		...
		//print_r($request->session()->get('paymentInput'));
		//print_r($request);
		 //die('asds');
