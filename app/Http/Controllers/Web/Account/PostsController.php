...
class PostsController extends AccountBaseController
{
	...
	{
		$pagePath = 'my-posts';
		$packages = Package::select("id","name","duration")->get();

		$colors = [1 => '#FF9933',2 => '#13C1AC',3=>'#8057a7',4=>'#B43A02',5=>'#2A3744',33=>'#FEE325',34=>'#2ECC71',35=>'#B43B02'];
	    ...
	    $total=mywallet::where('user_id','=',$user_id)->sum('price');
	    $data['total']=$total;
		return appView('account.posts', $data);
	}
	public function paymentclose(){
	    //$payments=Payment::where('active','=',1)->where('status','=',0)->get()->toArray();
	    $payments=Payment::where('active','=',1)->get()->toArray();
	    $now=time();
	    foreach($payments as $key=>$value){
	        $duration=$value['duration'];
	        $timeperiod=$duration*7+1;
	        $post_id=$value['post_id'];
	        $id=$value['id'];
	        $created_at=date('Y-m-d',strtotime($value['created_at']));
	        $current=date('Y-m-d',$now);
	        $created_at_last = date('Y-m-d', strtotime($value['created_at']. " + $timeperiod days"));
	       if($current>=$created_at_last){
	          //echo $post_id.' ';
	           Payment::where('id','=', $id)
                ->update(['active' => 0,'status'=>1]);
                Post::where('id','=', $post_id)
                ->update(['reviewed' => 0,'featured'=>0,'verified_top'=>1000]);
	       }
	...
	public function getwalletPosts()
	{
		$data = [];
		$data['posts'] = $this->pendingPosts->paginate($this->perPage);
