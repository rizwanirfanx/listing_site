 ...
 * @return string|null
 */
function uploadPostPicture($path, $file)
{
...
function genPhoneNumberBtn($post, $btnBlock = false)
{
	...
	$whatsAppLink = 'https://wa.me/' . strToDigit($post->phone) . $whatsAppPreFilledMessage .' with link '. url()->current();
	$waBtnClass = '';
...
function getFilterClearBtn($url)
{
	...
	if (!empty($url)) {
		...
		$out .= '<i class="far fa-window-close" style="float: right; margin-top: 6px; color: #999;"></i>';
		$out .= '</a>';
	}
