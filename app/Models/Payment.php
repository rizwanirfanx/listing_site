...
class Payment extends BaseModel
{
	 ...
	 * @var array
	 */
	protected $fillable = ['post_id', 'package_id', 'payment_method_id', 'transaction_id', 'amount', 'active','status'];
	...
	*/
	...
	public function getPackageNameHtml()
	{
		...
		$new_price=1;
		$foundPayments = Payment::where('post_id', $this->post_id)->first();


		// if(count($foundPayments)>0)
		// {
			if(!empty($foundPayments->duration))
		  	$new_price = $foundPayments->duration;
		// }
		...
		if (!empty($this->package)) {
			...
			$out .= '</a>';
			$out .= ' (' . $this->package->price * $new_price . ' ' . $this->package->currency_code . ')';
		}
		
		return $out;
	}

	public function gettotalWeekHtml()
	{	
		$out = 1 .' Week';
		$new_price=1;
		$foundPayments = Payment::where('post_id', $this->post_id)->first();
			...
			{
				$out = '';
				$out .= $foundPayments->duration.' Week';
			}
