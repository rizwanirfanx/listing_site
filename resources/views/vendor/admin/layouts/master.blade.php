            ...
            <div class="row">
                ...
                @if (config('settings.footer.hide_powered_by') != '1')
                    <div class="col-md-6 text-right">
                        ...
                        @else
                            {{ trans('admin.powered_by') }} <a target="_blank" href="http://zecanto.com">Zecanto</a>
                        @endif
                    </div>
                @endif
            </div>
...
<script>
    ...
    $(document).on('click', '.ajax-requestt', function(e) {
             
            e.preventDefault(); /* prevents the submit or reload */
            var confirmation = confirm("<?php echo trans('admin.confirm_this_action'); ?>");
            
            if (confirmation) {
                saveAjaxRequest2(siteUrl, this);
            }
        });
});

 function topup(num,id){
     var confirmation = confirm("<?php echo trans('admin.confirm_this_action'); ?>");
            
            if (confirmation) {
                saveAjaxRequest2(siteUrl,num, id)
            }
     
 }
    function saveAjaxRequest2(siteUrl, num,id)
    {
