@if (isset($selectedPackage, $paymentMethods) && !empty($selectedPackage) && $paymentMethods->count() > 0)
	...
	<p id="price-{{ $selectedPackage->id }}" class="hide">
		...
		@endif
		<?php
		if(!empty(request()->input('duration')) && request()->input('duration')>0)
		{		
			$duration = request()->input('duration');
			$selectedPackage->price = $selectedPackage->price * $duration;
		
		}

		if(!empty(request()->input('package')) && (request()->input('package') == 22 || request()->input('package') == 23 ))
		{		
			// $duration = request()->input('duration');
			// $selectedPackage->price = $selectedPackage->price * $duration;
